    ...
    public class eCommercialHubs : Hub
    {
        ...
        public void Like(int idproduct,string idcustomer)
        {
            ...
            if (dblike.Count() ==0)
            {
                ...
                toast = "Product " + db.Products.Find(idproduct).Product_Name + " successfully saved";
                isSave = true;
            }
            else
            {
                db.Product_Like.Remove(db.Product_Like.Find(dblike[0].Id));
                toast = "Product " + db.Products.Find(idproduct).Product_Name + " successfully deleted";
                isSave = false;
            }
            db.SaveChanges();
            ...
            Clients.All.ChangeLikeButton(countSavedProduct,toast,idproduct,isSave);
